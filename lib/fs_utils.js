...
const getUrlWithTrailingSlashIfNeeded = input => {
  if (pathIsRemoteUrl(input)) {
    let output = getSanitizedRemoteUrl(input);
    output = getPathWithoutTrailingSlash(output);
    const parsed = require(`url`).parse(input);
    // file with extension: no trailing slash
    const fileExtension = require(`path`).extname(parsed.path);
    if (fileExtension.length > 0 && fileExtension !== `.`) {
      if (parsed.query) {
        return `${output}?${parsed.query}`;
      }
      return `${output}`;
    }
    if (parsed.query) {
...
module.exports = {
  ...
  loadFile,
  mkdirpPromised,
  getUrlWithTrailingSlashIfNeeded,
  pathIsRemoteUrl,
  rimrafPromised,
  statPromised,
  writeFile
