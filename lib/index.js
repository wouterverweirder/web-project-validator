...
const getProjectsForInputPath = (inputPath, inputType = false) => {
  ...
  .then(inputType => {
    if (inputType === `list`) {
      return fsUtils
        ...
        .then(urls => urls.map(url => {
          return {
            url: fsUtils.getPathWithProtocol(url)
          };
        }));
      ...
      .then(projectMetaDatas => {
        ...
        projectMetaDatas.forEach(projectMetaData => {
          if (projectMetaData.online) {
            projects.push({
              url: fsUtils.getPathWithProtocol(projectMetaData.online),
              github: projectMetaData.github
            });
          } else {
            projectMetaData.htmls.forEach(htmlFile => {
              projects.push({
                url: fsUtils.getPathWithProtocol(htmlFile)
              });
            });
          }
        });
    ...
    const url = fsUtils.getPathWithProtocol(inputPath);
    return [{
      id: url,
      url
    }];
