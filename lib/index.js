...
const getGlob = (inputFolder, type) => {
  ...
  case `url`: return `${inputFolder}/**/@(url|link|github|git)(.txt|.rtf|.rtfd|.md)`;
  case `url2`: return `${inputFolder}/**/*snelkoppeling*`;
  case `webloc`: return `${inputFolder}/**/*.webloc`;
  case `gitConfig`: return `${inputFolder}/**/.git/config`;
  case `html`: return `${inputFolder}/**/*@(.htm|.html)`;
...
const findByKind = (kind, directoryPath) => {
  return new Promise(resolve => {
    ...
    exec(`mdfind 'kMDItemKind="${kind}"' -onlyin '${directoryPath}'`, (error, stdout) => {
      if (error) {
    ...
    .then(() => {
      metaData.urls.forEach(url => {
        ...
        if (domain === `github.com`) {
          metaData.github = url;
        } else if (domain === `youtu.be` || domain === `git.heroku.com` || domain === `pinterest.com`) {
          // ignore
        } else {
          if (!metaData.online) {
            metaData.online = url;
...
const getUrlsFromFileKind = (kind, directoryPath, urls = []) => {
  return Promise.resolve()
    .then(() => findByKind(kind, directoryPath))
    .then(files => getUrlsFromUrlFiles(files, urls));
};

const getUrlsFromGlobMatches = (pattern, globOptions, urls = []) => {
  return Promise.resolve()
    .then(() => globPromised(pattern, globOptions))
    .then(files => getUrlsFromUrlFiles(files, urls));
};
...
const getUrlsForProjectPath = projectPath => {
  ...
  let urls = [];
  return Promise.resolve()
    .then(() => getUrlsFromGlobMatches(getGlob(projectPath, `url`), globOptions, urls))
    .then(() => getUrlsFromGlobMatches(getGlob(projectPath, `url2`), globOptions, urls))
    .then(() => getUrlsFromGlobMatches(getGlob(projectPath, `webloc`), globOptions, urls))
    .then(() => getUrlsFromGlobMatches(getGlob(projectPath, `gitConfig`), globOptions, urls))
    .then(() => getUrlsFromFileKind(`Web internet location`, projectPath, urls))
    .then(() => {
      urls = urls
      .map(url => {
...
const getUrlsFromUrlFiles = (urlFilePaths, urls = []) => {
  return Promise.resolve()
    .then(() => Promise.all(urlFilePaths.map(filePath => getUrlsFromUrlFile(filePath))))
...
const getUrlsFromUrlFile = urlFilePath => {
  ...
  return new Promise(resolve => {
    ...
    plist.readFile(urlFilePath, (err, data) => {
      if (!err) {
