...
var generateReport = function(cssFilePath) {
  var csslint = require('csslint').CSSLint;
  return new Promise(function(resolve, reject){
    fs.readFile(cssFilePath, 'utf-8', function(err, result){
      if(err) {
        throw err;
        return;
      }
      var csslintResult = csslint.verify(result, getRules());
      csslintResult.context = cssFilePath;
      csslintResult.warningMessages = [];
      csslintResult.dangerMessages = [];
      csslintResult.messages.forEach(function(message){
        if(message.type === 'error') {
          message.outputType = 'danger';
          csslintResult.dangerMessages.push(message);
        } else {
          message.outputType = 'warning';
          csslintResult.warningMessages.push(message);
        }
      });
