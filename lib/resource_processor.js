...
const processRequestedResource = (report, url, resource) => {
  return new Promise(resolve => {
    ...
    if (resource.type === `style` || resource.type === `html` || resource.type === `javascript`) {
      // ignore files on certain domains
      const domain = fsUtils.getUrlDomain(resource.url);
      const ignoredDomains = ['fonts.googleapis.com', 'use.typekit.net', 'kit.fontawesome.com', 'cdnjs.cloudflare.com'];
      if (ignoredDomains.indexOf(domain) === -1) {
