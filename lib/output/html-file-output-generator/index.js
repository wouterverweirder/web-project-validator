...
const generateOutput = (report, htmlFilePath, options) => {
  ...
  let seq = Promise.resolve();
  {
    ...
    .then(contents => {
      const templateSource = '<textarea style="width: 100%; height: 600px;" data-mode="{{mode}}" data-file="{{context}}">{{code}}</textarea>';
      const template = require('handlebars').compile(templateSource);
      return template({code: contents, mode: resourceReport.mode, context: resourceReport.context});
    })
